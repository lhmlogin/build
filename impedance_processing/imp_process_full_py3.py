...
from PyQt5.QtWidgets import QHeaderView,QAbstractItemView,QFileDialog,QMessageBox,QMenu
...
#         self.setupUi(self)
        ...
        self.realModel.setHorizontalHeaderLabels([u"类型",u"层",u"顶层参考层",u"底层参考层",u"线宽(mil)",u"目标线宽(mil)",u"间距(mil)",u"目标间距(mil)",u"阻值(Ω)"])
    ...
    def createImpList(self):
        ...
        for i in range(row):
            ...
            tmpImpDict['line_width']=float(tmpImpDict['line_width'])/1000
            tmpImpDict['req_line_width']=float(tmpImpDict['req_line_width'])/1000
            tmpImpDict['spacing']=float(tmpImpDict['spacing'])/1000
            tmpImpDict['req_spacing']=float(tmpImpDict['req_spacing'])/1000
    ...
    def runILR(self):
        ...
        f.COM("chklist_single,action=frontline_imp_rec_simp,show=no")
        # f.COM("imp_refresh_tbl,refresh_table=yes")
        # f.COM("imp_cnsts_change,imp_id=1,ilr_dfm_use=yes")
        # f.COM("imp_cnsts_change,imp_id=2,ilr_dfm_use=yes")
        # f.COM("imp_cnsts_change,imp_id=1,req_spacing=0.0075")
        # f.COM("imp_refresh_tbl,refresh_table=yes")
        # QMessageBox.information(self,u"info",u"impedance line recoginization is finished",QMessageBox.Ok)
        ...
        for imp in self.impList:
            ...
            if selectCount != 0:
                ...
                if imp['type'] == 'differential':
                    ...
                    impLayerName=str(imp['id'])+"_imp-"+imp['lyr']+"-"+imp['imp_value']+"ohm-diff-"+imp['line_width_mil']+"-"+imp['spacing_mil']
                    ...
                    impLayerName=str(imp['id'])+"_imp-"+imp['lyr']+"-"+imp['imp_value']+"ohm-sing-"+imp['line_width_mil']
                    info=f.DO_INFO("-t layer -e %s/%s/%s -d EXISTS"%(jobName,stepName,impLayerName))
                    if info['gEXISTS'] == "yes":
                        f.COM("delete_layer, layer=%s"%(impLayerName))
                    f.COM("sel_copy_other,dest=layer_name,target_layer=%s,invert=no,dx=0,dy=0,size=0,x_anchor=0,y_anchor=0"%(impLayerName))
    ...
    def runImpIndexing(self):
        ...
        for imp in self.impList:
            ...
            if info['gEXISTS'] == "yes":
                ...
                f.COM("chklist_cupd,chklist=frontline_impline_indexing,nact=1,params=((pp_ref_layer=%s)(pp_copper_layer=%s)(pp_imp_id=%s)(pp_type=%s)(pp_spacing=%s)(pp_spacing_tol=0.1)),mode=regular"%(impLayerName,imp['lyr'],imp['id'],imp['type'],imp['spacing_mil']))
    ...
    def createTP(self):
        ...
        f.COM("chklist_cupd,chklist=frontline_imp_test_points_creator,nact=1,params=((pp_tp_min_width=%s)(pp_tp_max_width=%s)(pp_imp_min_len=%s)(pp_imp_min_coverage=%s)(pp_tp_good_pair_num=1)(pp_pgtp_max_search_dist=%s)(pp_pgtp_min_width=%s)(pp_allow_middle_pad=allow_middle_pad)(pp_imp_min_avail_percent=10.000000)(pp_imp_has_best_len=)(pp_imp_best_len=4)(pp_tp_need_middle=%s)(pp_tp_need_shortest=%s)(ignore_attrs=.gold_finger)),mode=regular"%(paramDict['tp_min_width'],paramDict['tp_max_width'],paramDict['tp_imp_min_len'],paramDict['tp_min_coverage_percent'],paramDict['tp_pgpad_max_dist'],paramDict['tp_min_width'],need_shortest,need_middle))
        ...
        for imp in self.impList:
            ...
            for tp_id_num in tp_id_num_list:
                ...
                if tp_id_num in tp_dict[outerLayer].keys():
                ...
                if selectCount != 0:
                    ...
                    if imp['type'] == 'differential':
                        tpLayerName=str(imp['id'])+"_tp_"+outerLayer+"-"+imp['lyr']+"-"+imp['imp_value']+"ohm-diff-"+imp['line_width_mil']+"-"+imp['spacing_mil']
                    elif imp['type'] == 'single-ended':
                        tpLayerName=str(imp['id'])+"_tp_"+outerLayer+"-"+imp['lyr']+"-"+imp['imp_value']+"ohm-sing-"+imp['line_width_mil']
            ...
            if tp_id_loop == 1 and imp['type'] == 'single-ended':
                pgtpLayerName=str(imp['id'])+"_tp_"+outerLayer+"-"+imp['lyr']+"-"+imp['imp_value']+"ohm-sing-"+imp['line_width_mil']+"-pglayer"
        ...
        for imp in self.impList:
            if imp['has_tp']==1:
                ...
                for i in range(len(imp['tpLimitMinX'])):
                    ...
                    f.COM("zoom_pv_open,x1=%s,y1=%s,x2=%s,y2=%s,x_win=0,y_win=%s,w_win=300,h_win=260"%(imp['tpLimitMinX'][i]-0.05,imp['tpLimitMinY'][i]-0.05,imp['tpLimitMaxX'][i]+0.05,imp['tpLimitMaxY'][i]+0.05,i*300))


                bdnetStr="\\;".join(imp['tp_bdnet'])
                f.COM("display_net,job=%s,step=net,netlist=cur,color=5,section=first,nets=%s\;,disp_mode=outline,all_lyrs=yes,top_tp=no,bot_tp=no,drl_tp=no,npth_tp=no,inner_tp=no"%(jobName,bdnetStr))
                ...
                if imp['type'] == 'differential':
                    note1=imp['top_ref']+"/"+imp['lyr']+"/"+imp['bot_ref']+"-diff-"+imp['line_width_mil']+"/"+imp['spacing_mil']+"-"+imp['imp_value']+"ohm"+"-"+imp['impLen'][0]+"inch"
                elif imp['type'] == 'single-ended':
                    note1=imp['top_ref']+"/"+imp['lyr']+"/"+imp['bot_ref']+"-single-"+imp['line_width_mil']+"-"+imp['imp_value']+"ohm"+"-"+imp['impLen'][0]+"inch"
    ...
    def createPDF(self):
        for imp in self.impList:
            if imp['has_tp']==1:
                ...
                if imp['type'] == 'differential':
                    type_cn=u"差分阻抗"
                    orig_width=imp['line_width_mil']+"/"+imp['spacing_mil']
                    req_width=imp['req_line_width_mil']+"/"+imp['req_spacing_mil']
  
                elif imp['type'] == 'single-ended':
                    type_cn=u"单端阻抗"
                    orig_width=imp['line_width_mil']
                    req_width=imp['req_line_width_mil']
                ...
                data= [[u"序号",u"类型",u"成品Ω",u"层别",u"上参考层",u"下参考层",u"原稿mil",u"成品mil",u"视图面次",u"1线长/覆盖率","",u"2线长/覆盖率","",u"3线长/覆盖率",""],
                ...
                data= [[u"序号",u"类型",u"成品Ω",u"层别",u"上参考层",u"下参考层",u"原稿mil",u"成品mil",u"没有合适测点",u"可能原因"],
                        [imp['id'],type_cn,finishValue,imp['lyr'],imp['top_ref'],imp['bot_ref'],orig_width,req_width,u"没有合适测点",""]]
                t = Table(data, style={
                    ("FONT", (0, 0), (-1, -1), msyh, 8),
    ...
    def pause(self):
        ...
        self.show()
        self.move((QtWidgets.QApplication.desktop().width() - window.width())/2,(QtWidgets.QApplication.desktop().height() - window.height())/2)
    ...
    def load_impList(self,impFilePath):
        ...
        for impLine in impLineList:
            ...
            for j in range(len(self.keyNameList)):
                value=lineArr[j]
                if value == "\\" or value == "0.00" or value=="0":
                    value=""
                    # if j <4 :
                    #     value=""
                    # else:
...
#         self.setupUi(self)
    ...
    def setupUi(self, Dialog):
        ...
        self.scrollAreaWidgetContents.setGeometry(QtCore.QRect(0, 0, 659, 1003))
        ...
        self.verticalLayout_4.addWidget(self.groupBox_ilr)
        ...
        self.verticalLayout_4.addWidget(self.groupBox_tp)
        ...
        self.verticalLayout_4.addWidget(self.groupBox_report)
        ...
        self.verticalLayout_4.addWidget(self.groupBox_ila)
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label.setText(_translate("Dialog", "差分阻抗最短长度(mil):"))
        ...
        self.label_3.setText(_translate("Dialog", "线宽最大差异(mil):"))
        self.lineEdit_trace_width_tol.setToolTip(_translate("Dialog", "阻抗线线宽误差，大于或小于此误差值不会识别为对应类型阻抗线"))
        self.label_4.setText(_translate("Dialog", "线距最大差异(mil):"))
        ...
        self.label_5.setText(_translate("Dialog", "测试阻抗线最小长度(inch):"))
        ...
        self.label_7.setText(_translate("Dialog", "特性阻抗接地测点距目标测点最大距离(mil):"))
        self.checkBox_need_shortest.setText(_translate("Dialog", "需识别最短线"))
        self.checkBox_need_middle.setText(_translate("Dialog", "需识别中值线"))
        self.label_14.setText(_translate("Dialog", "测点pad短边最大长度(mil):"))
        self.label_8.setText(_translate("Dialog", "测点pad短边最小长度(mil):"))
        ...
        self.label_10.setText(_translate("Dialog", "最小线路距离(mil):"))
        self.label_11.setText(_translate("Dialog", "最小钻孔距离(mil):"))
        self.label_12.setText(_translate("Dialog", "间距额外识别距离(mil):"))
